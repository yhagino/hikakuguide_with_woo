...
def get_from_ssh(select_columns):
    sql = f'SELECT {",".join(select_columns)} FROM wp_posts WHERE post_name REGEXP "^[0-9]*[a-z][-]*"'
    print(sql)
...
def add_product(list_columns, list_data):
    ...
    sql = "".join(["INSERT INTO wp_posts (", ",".join(list_columns), ")", f" VALUES({('%s, ' * len(list_columns)).rstrip(', ')})"])
    print(sql)
...
if __name__ == '__main__':
    ...
    ssh_data = get_from_ssh(target_data)
    ssh_data.to_csv('../../jupyter/ssh_data.csv')
    print(ssh_data)
    # sql = add_product()
